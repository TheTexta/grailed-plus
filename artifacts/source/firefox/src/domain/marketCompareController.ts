...
(function (root: MCGlobalRoot, factory: () => MCModule) {
  ...
  function createController(options?: unknown): MCController {
    ...
    const rankCandidates =
      typeof config.rankCandidates === "function"
        ...
        : MatchScoring && typeof MatchScoring.rankCandidates === "function"
          ...
          : null;
    const rankCandidatesAsync =
      typeof config.rankCandidatesAsync === "function"
        ? config.rankCandidatesAsync
        : typeof config.rankCandidates === "function"
          ? null
        : MatchScoring && typeof MatchScoring.rankCandidatesAsync === "function"
          ? MatchScoring.rankCandidatesAsync
          : null;
    ...
    function runRankCandidates(
      listing: MCListing | null,
      filteredCandidates: MCCandidate[],
      rankingOptions: unknown
    ): Promise<MCCandidate[]> {
      if (rankCandidatesAsync) {
        return Promise.resolve(rankCandidatesAsync(listing, filteredCandidates, rankingOptions)).then(function (
          result
        ) {
      ...
      return Promise.resolve(rankCandidates(listing, filteredCandidates, rankingOptions)).then(function (result) {
        return Array.isArray(result) ? result : [];
      });
    }
    ...
    function rankFilteredCandidates(
      ...
      return runRankCandidates(listing, filteredCandidates, strictRankingOptions).then(function (rankedCandidates) {
        if (rankedCandidates.length > 0 || filteredCandidates.length === 0) {
