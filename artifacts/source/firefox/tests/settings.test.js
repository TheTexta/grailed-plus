...
function createBrowserStorage(initialState) {
  const state = Object.assign({}, initialState || {});
...
test("settings use browser.storage promise APIs when chrome is unavailable", async () => {
  const previousChrome = global.chrome;
  const previousBrowser = global.browser;

  const browserMock = createBrowserStorage({});
  global.chrome = undefined;
  global.browser = browserMock;

  try {
    const currencyResult = await setSelectedCurrency("cad");
    assert.deepEqual(currencyResult, { ok: true });
    assert.equal(browserMock.__state[CURRENCY_STORAGE_KEY], "CAD");
    assert.equal(await getSelectedCurrency(), "CAD");

    const conversionResult = await setCurrencyConversionEnabled(true);
    assert.deepEqual(conversionResult, { ok: true });
    assert.equal(browserMock.__state[CONVERSION_ENABLED_STORAGE_KEY], true);
    assert.equal(await getCurrencyConversionEnabled(), true);
  } finally {
    global.chrome = previousChrome;
    global.browser = previousBrowser;
...
test("getMarketCompareEnabled removes legacy cached setting during cutover", async () => {
  const previousChrome = global.chrome;
  const previousBrowser = global.browser;

  const chromeMock = createChromeStorage({
    grailed_plus_market_compare_enabled_v1: false
  });
  global.chrome = chromeMock;
  global.browser = undefined;
  ...
  try {
    ...
    assert.equal(enabled, DEFAULT_MARKET_COMPARE_ENABLED);
    assert.equal(enabled, true);
    assert.equal(chromeMock.__state.grailed_plus_market_compare_enabled_v1, undefined);
    assert.equal(chromeMock.__state[MARKET_COMPARE_ENABLED_STORAGE_KEY], undefined);
  } finally {
    global.chrome = previousChrome;
    global.browser = previousBrowser;
  }
