...
const IMAGE_MESSAGE_TYPE = "grailed-plus:image-fetch";
...
function loadBackgroundWithMocks(options = {}) {
  ...
  const runtimeNamespace = options.runtimeNamespace === "browser" ? "browser" : "chrome";
  const runtimeMock = {
    runtime: {
      onMessage: {
        addListener(fn) {
  ...
  global.chrome = runtimeNamespace === "chrome" ? runtimeMock : undefined;
  global.browser = runtimeNamespace === "browser" ? runtimeMock : undefined;
...
test("background listener registers against browser.runtime when chrome is unavailable", async () => {
  const { listener, fetchCalls, restore } = loadBackgroundWithMocks({
...
test("background listener returns image bytes for valid image fetch requests", async () => {
  const observedFetches = [];
  const expectedBytes = Uint8Array.from([137, 80, 78, 71]).buffer;
  const { listener, restore } = loadBackgroundWithMocks({
    fetchImpl: async function (url, options) {
      observedFetches.push({ url, options });
      return {
        ok: true,
        status: 200,
        headers: {
  ...
  try {
      ...
      const keepAlive = listener(
        {
          ...
          url: "https://images.depop.com/listings/example.png"
        },
        VALID_SENDER,
        sendResponse
      );
    ...
    assert.equal(response.bytes.byteLength, expectedBytes.byteLength);
    assert.equal(observedFetches.length, 1);
    assert.equal(observedFetches[0].options.credentials, "omit");
  } finally {
    restore();
  }
});
