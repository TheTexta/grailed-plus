...
(function (root, factory) {
    ...
    function sendRuntimeMessage(message) {
        ...
        return new Promise(function (resolve) {
            ...
            try {
                const maybePromise = sendMessage(message, function (response) {
    ...
    async function fetchImageBlobViaRuntime(url) {
        if (!/^https:\/\//i.test(url)) {
            return null;
        }
    ...
    function compareHashes(leftHash, rightHash) {
...
(function (root, factory) {
    ...
    "use strict";
    const IMAGE_HEURISTIC_STOPWORDS = {
        asset: true,
        assets: true,
        cache: true,
        cached: true,
    ...
    function isHashLikeToken(token) {
    ...
    function shouldSkipImageToken(token) {
        if (!token) {
            return true;
        }
    ...
    function extractImagePathSignal(url) {
        ...
        const segments = pathname
            ...
            const withoutExtension = index === all.length - 1 ? decoded.replace(/\.[a-z0-9]{2,5}$/i, "") : decoded;
            return uniqueTokens(withoutExtension
                .replace(/[_+]+/g, "-")
                .split(/[^a-z0-9]+/g)
                .filter(Boolean)
                .filter(function (token) {
        ...
        const pathTokens = uniqueTokens(segments.slice(0, Math.max(0, segments.length - 1)).reduce(function (all, entry) {
    ...
    function buildListingImageUrl(listing) {
        return (normalizeString(listing && listing.imageUrl, "") ||
    ...
    function scoreBrand(listing, candidate) {
        const left = normalizeComparableText(listing && (listing.brand || (listing.rawListing && listing.rawListing.brand)));
    ...
    function scoreSize(listing, candidate) {
        const left = normalizeComparableText(listing && (listing.size || (listing.rawListing && listing.rawListing.size)));
    ...
    function scorePriceDelta(listingComparable, candidateComparable) {
        if (listingComparable == null ||
    ...
    function computeWeightedScore(rankingFormula, imageHeuristicScore, titleScore, brandScore, sizeScore, conditionScore, priceScore) {
        const signalScores = {
    ...
    function scoreCandidateFromImageResult(listing, candidate, config, imageResult) {
    ...
    async function rankCandidatesAsync(listing, candidates, options) {
        const list = Array.isArray(candidates) ? candidates : [];
        const config = options && typeof options === "object" ? options : {};
        const minScore = Number.isFinite(Number(config.minScore)) ? Number(config.minScore) : 0;
        const scored = await Promise.all(list.map(function (candidate) {
    ...
    function createController(options) {
        ...
        const rankCandidates = typeof config.rankCandidates === "function"
            ...
            : MatchScoring && typeof MatchScoring.rankCandidates === "function"
                ...
                : null;
        const rankCandidatesAsync = typeof config.rankCandidatesAsync === "function"
        ...
        function runRankCandidates(listing, filteredCandidates, rankingOptions) {
            if (rankCandidatesAsync) {
                return Promise.resolve(rankCandidatesAsync(listing, filteredCandidates, rankingOptions)).then(function (result) {
